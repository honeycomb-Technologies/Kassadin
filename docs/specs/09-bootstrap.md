...
- Fuller Haskell-style `Accounts` / `DState` parity beyond the current per-credential registration mirror, pre-Conway pointer-stake instant-stake tracking, modern-era governance/Conway parameter changes, modern min-ADA/cost-model handling, full epoch reward/state maintenance, and final ledger snapshot/checkpoint persistence on shutdown are still pending
