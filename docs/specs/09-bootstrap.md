...
- `bootstrap-sync` resolves the snapshot root from `db/<network>/`, reads the snapshot tip, loads the latest local ledger snapshot from `ledger/<slot>/tables/tvar`, hydrates reward balances, stake deposits, `utxosDeposited`, current/future pool params, current/future pool owner memberships, current/future genesis delegations from `DState`, pool reward accounts, pool retirements, chain-account pots, fee pots, rollback-safe pending MIR state, Haskell-shaped `BlocksMade` (`nesBprev` / `nesBcur`), and Haskell-shaped mark/set/go stake snapshots from `ledger/<slot>/state`, replays the immutable tail to the immutable tip, anchors `ChainDB` there, and fetches/parses real forward blocks with local validation enabled
...
- The CLI can also load relay peers from official topology JSON (`Producers`, `bootstrapPeers`, `localRoots`, `publicRoots`), preserve the full resolved relay list for `sync` / `bootstrap-sync`, and fail over sequentially across that list on connect/reconnect
...
- Bootstrap/runtime snapshot state now hydrates reward balances, stake deposits, `utxosDeposited`, current/future pool state, current/future pool owner memberships, current/future genesis delegations from `DState`, chain-account pots, fee pots, rollback-safe pending MIR state, Haskell-shaped `BlocksMade` (`nesBprev` / `nesBcur`), and modern Haskell `SnapShots = [mark,set,go,fee]` stake snapshots before immutable-tail replay; observed 9-field pool entries are handled compatibly by deriving active stake from the outer active-stake maps, account-state import now goes through one explicit stake-account path so registered empty accounts survive ancillary hydration, local ledger checkpoints now preserve that unified stake-account state directly so registered empty accounts also survive checkpoint reload, ancillary pre-Conway account-state pointers now hydrate into local stake-account state, mark/set/go now retain credential-level active stake plus the snapshot-era pool reward account and self-delegated owner state locally, live/snapshot UTxO entries now retain either staking credentials or pointer refs, rollback preserves those stake refs, and epoch snapshot rotation resolves active stake from the unified local stake-account state so pointer-backed stake and reward-account balances flow through the same Haskell-style instant-stake path. Pre-Conway snapshot rotation still resolves pointer-backed stake, but Conway-era snapshot rotation disables pointer-backed instant stake before epoch-boundary processing. Shelley genesis now seeds the initial genesis-delegation map locally, tag-5 genesis-delegation certs stage future delegations at `slot + stabilityWindow`, and slot ticking adopts the latest matured delegation per genesis key before block validation. Pool re-registration now stages future params and future owner sets locally instead of mutating live pool state immediately, those staged values activate at epoch processing, MIR now realizes-or-drops against local pots and registered reward credentials at epoch boundaries, the deposited pot is now imported from snapshot `UTxOState`, adjusted by certificate deposits/refunds, checkpointed, and decremented during pool reap, and the current epoch reward path now uses imported/rotated `BlocksMade`, Shelley genesis `activeSlotsCoeff`, and a Shelley `maxPool'`-style pool-pot calculation to credit delegator reward accounts as well as pool reward accounts while carrying the epoch balance sheet through explicit `deltaT` / `deltaR` / `deltaF` updates, filtering payouts against currently registered reward credentials, routing unclaimable rewards to treasury, enforcing snapshot-era pledge coverage, and using Shelley genesis reward parameters during immutable replay and live epoch-boundary processing
- Once local ledger validation is enabled, reward balances are treated as tracked local state throughout follower operation, so withdrawals must match the locally stored reward balance exactly instead of being accepted without proof
...
- Snapshot-backed follower starts now seed Praos/TPraos state from the Shelley genesis file hash, distinguish Shelley-Alonzo TPraos epoch ticks from Babbage/Conway Praos epoch ticks, and persist snapshot-tip chain-dependent nonce state in `db/<network>/praos.resume` so repeated restarts can skip the full immutable Praos rebuild once that sidecar exists
- Fuller Haskell-style `Accounts` / `DState` parity beyond the current per-credential registration mirror, modern-era governance/Conway parameter changes, modern min-ADA/cost-model handling, full epoch reward/state maintenance, and final ledger snapshot/checkpoint persistence on shutdown are still pending
